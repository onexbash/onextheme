...
func GetColors() {
	for _, flavour := range []catppuccin.Flavour{
		...
		fmt.Println(lipgloss.NewStyle().Bold(true).Render(strings.ToUpper(flavour.Name())))
		var rosewater catppuccin.Color = flavour.Rosewater()
		fmt.Println(rosewater)
		fmt.Println(rosewater.HSL)
